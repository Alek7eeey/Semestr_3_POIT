	...
	void WriteOutFile(int argc, wchar_t* argv[], LT::LexTable& lextable, IT::IdTable& idtable)
	{
		...
		for (int i = 0; i < idtable.size; i++)
		{
			if (idtable.table[i].iddatatype == 1)
			{
				...
				{
					Out << "Тип данных: NUMBER " << "\t идентификатор: " << idtable.table[i].id << "\t строка: " << idtable.table[i].idxfirstLE
			...
			if (idtable.table[i].iddatatype == 2)
			{
				if (idtable.table[i].idtype == 1)
				{
					Out << "Тип данных: LINE " << "\t идентификатор: " << idtable.table[i].id << "\t строка: " << idtable.table[i].idxfirstLE
